...
> GMP wrapper, Binary Splitting의 P/Q/T 구조와 merge는 구현되어 있지만,
> Chudnovsky leaf 계산과 실제 병렬 Binary Splitting은 아직 연결되지 않았습니다.
...
현재 이 P/Q/T 자료구조, 재귀 분할 API, merge 식은 구현되어 있습니다.
Chudnovsky leaf 값과 잘못된 `[start, end)` 범위 검증은 다음 구현 단계이며,
cutoff 기반 scheduler 병렬 실행은 그 다음 단계입니다.
...
| Binary Splitting | 골격 완료 | P/Q/T node, merge, sequential/parallel API |
| Chudnovsky leaf | 다음 단계 | leaf 공식, 범위 검증, known small result |
| 병렬 Binary Splitting | 계획됨 | cutoff 기반 task 제출과 child handle join |
